              ...
              <div className="p-5 md:p-7">
                ...
                {act===p.id ? (
                  <div className="space-y-3">
                    ...
                    {store && <div className="rounded-xl border border-rose-gold/20 bg-rose-gold/5 p-3"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-[10px] text-champagne/75">When you are ready, share this response together.</p><button type="button" onClick={() => store.markPromptReady(store.activePartner)} className="btn-ghost rounded-lg px-3 py-2 text-[9px] uppercase tracking-[0.08em]">Mark ready to share</button></div><p className="mt-2 text-[9px] text-champagne/60" aria-live="polite">{store.promptReveal.partnerAReady && store.promptReveal.partnerBReady ? 'Both partners are ready. Reveal when you are together.' : 'Waiting for both partners to mark ready.'}</p>{store.promptReveal.partnerAReady && store.promptReveal.partnerBReady && (store.promptReveal.partnerA[String(p.id)] || store.promptReveal.partnerB[String(p.id)]) && (store.promptReveal.revealed[String(p.id)] ? <div className="mt-3 grid gap-2 md:grid-cols-2"><p className="rounded-lg bg-black/15 p-3 text-xs text-champagne/95">{store.promptReveal.partnerA[String(p.id)] || 'Partner one shared no response.'}</p><p className="rounded-lg bg-black/15 p-3 text-xs text-champagne/95">{store.promptReveal.partnerB[String(p.id)] || 'Partner two shared no response.'}</p></div> : <button type="button" onClick={() => store.revealPromptAnswer(String(p.id))} className="btn-primary mt-3 rounded-lg px-3 py-2 text-[9px] uppercase tracking-[0.08em]">Reveal writing together</button>)}</div>}
                    <button onClick={()=>setAct(null)} className="text-[9px] text-champagne/75">Close</button>
                  </div>
                ) : (
